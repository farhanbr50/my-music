cat > ChandMusic/utils/db.py << 'EOF'
# --------------------------------------------------------------------------------
#  ✨ ChandMusic © 2026 ✨
...
def set_nsfw_enabled(chat_id: int, enabled: bool) -> None:
    ...
    try:
        col.update_one(
            {"_id": chat_id},
            {"$set": {"_id": chat_id, "enabled": enabled}},
            upsert=True,
        )
    except Exception as e:
...
def disapprove_nsfw_user(chat_id: int, user_id: int) -> None:
    ...
    if col is None:
        return
    try:
        col.delete_one({"_id": f"{chat_id}_{user_id}"})
    except Exception as e:
...
def is_nsfw_approved(chat_id: int, user_id: int) -> bool:
    ...
    if col is None:
        return False
    try:
        return col.find_one({"_id": f"{chat_id}_{user_id}"}) is not None
    except Exception:
        return False
...
def get_nsfw_approved_users(chat_id: int) -> list:
    ...
    if col is None:
        return []
    try:
        docs = col.find({"chat_id": chat_id}, {"user_id": 1})
        return [doc["user_id"] for doc in docs]
    except Exception:
        return []


def save_welcome_state(chat_id: int, enabled: bool) -> None:
    col = _col("welcome_state")
    if col is None:
        return
    try:
        col.update_one(
        ...
        logger.error(f"⚡ [DB] save_welcome_state: {e}")


def load_welcome_state(chat_id: int) -> bool:
    col = _col("welcome_state")
    if col is None:
        return True
    try:
